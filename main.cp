...
int main () {
	...
	while (gets(line)) {
		...
		if (command == 'Q') {
			...
			void* status;
			fprintf (log_file, "Attempting to join thread for query: (%d,%d)\n", node_map[x], node_map[y]);
			int st = pthread_join(*pt, &status);
			while (st) {
				fprintf (log_file, "Unable to join thread for query: (%d,%d), error = %d\n", node_map[x], node_map[y], st);
				st = pthread_join(*pt, &status);
			}
			free(pt);
			fprintf (log_file, "Successfuly joined thread for query: (%d,%d)\n", node_map[x], node_map[y]);
		} else if (command == 'A') {
			lock_for_write ();
